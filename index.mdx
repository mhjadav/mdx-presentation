...
import style from 'react-syntax-highlighter/src/styles/prism/ghcolors'
...
<Head>
  <title>E2E testing with Cypress & Bitbucket Pipelines @ Noon Academy</title>
</Head>
...
2. Validate critical flows (like payment, groups, classroom)
...
2.  Built on top of Mocha, jQuery 
...
2. Run only if PR created/updated to release (destination branch) Branch

3. Using start-server-and-test(node_modules), start server with updated code locallly
    <i>run test, generate report, record test result</i>

4. Generate failed test case xml reports
<i>Sample report</i>
<pre>
<code>
...
2.  Teacher - Create group (subject, test, skills, university, private, public)

3.  Student - Group details test, skill, private

4.  Student - Editorial detail page (free, paid)
